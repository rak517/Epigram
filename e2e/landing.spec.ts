...
test.describe('랜딩 페이지', () => {
  test('랜딩 페이지 로드 확인', async ({ page }) => {
    await page.goto('/');
    await expect(page).toHaveURL('http://localhost:3000');
  });

  test('로그인이 안되어 있으면, 검색 아이콘 클릭 시 로그인 페이지로 이동한다', async ({ page }) => {
    await page.goto('/');
    await page.getByAltText('검색 아이콘').click();
    await expect(page).toHaveURL('http://localhost:3000/login');
  });

  test('로고 클릭 시 랜딩 페이지로 이동한다.', async ({ page }) => {
    await page.goto('/');
    await page.getByAltText('헤더 로고').click();
    await expect(page).toHaveURL('http://localhost:3000');
  });

  test('로그인이 안되어 있으면, 유저 아이콘 클릭 시 로그인 페이지로 이동한다.', async ({ page }) => {
    await page.goto('/');
    await page.getByAltText('유저 아이콘').click();
    await expect(page).toHaveURL('http://localhost:3000/login');
  });

  test('로그인이 안되어 있으면, 시작하기 텍스트 클릭 시 로그인 페이지로 이동한다', async ({ page }) => {
    await page.goto('/');
    await page.getByRole('link', { name: '시작하기' }).first().click();
    await expect(page).toHaveURL('http://localhost:3000/login');
  });

  test('로그인이 안되어 있으면, 하단의 시작하기 텍스트 클릭 시 로그인 페이지로 이동한다', async ({ page }) => {
    await page.goto('/');
    await page.getByRole('link', { name: '시작하기' }).nth(1).click();
    await expect(page).toHaveURL('http://localhost:3000/login');
  });

  test('더 알아보기 텍스트 클릭 시 기능 소개 레이아웃으로 스크롤이 내려간다', async ({ page }) => {
  ...
  test('하단 화살표 이미지 클릭 시 기능 소개 레이아웃으로 스크롤이 내려간다', async ({ page }) => {
    const initialScrollY = await page.evaluate(() => window.scrollY);
