...
interface TextCardServerProps extends VariantProps<typeof textCardVariants> {
  cardContent?: string;
  author?: string;
  tags?: Tag[];
  maxTags?: number;
...
export default function TextCardServer({
  cardContent,
  author,
  tags = [],
  maxTags = 2,
    ...
    return (
      ...
      >
        ...
        {tags.length > 0 && (
          ...
          >
            {displayTags.map((tag) => (
              <span key={tag.id} className={`py-1 text-blue-400 ${tagClassName}`}>
