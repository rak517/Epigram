...
export default function LandingHeader({ showIcon = false }: LandingHeaderProps) {
  ...
  if (showIcon) {
    return (
      <header className='border-b-line-100 fixed z-10 flex w-full items-center justify-between border-b bg-white px-6 py-2 md:h-[60px] md:px-[72px] md:py-3 xl:h-20 xl:px-[120px] xl:py-4'>
        <Link href={'/search'}>
        ...
        <DropdownMenu size='sm' options={options} onSelect={handleSelect} trigger={<Image src={userIconDark} alt='유저 아이콘' />} />
      </header>
    );
  }
